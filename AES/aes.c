...
void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) {
    ...
    // Variables
    unsigned int bytesGenerated = 16; // We already generated 16 bytes 
...
void AES_Encrypt(unsigned char* messsage, unsigned char* key) {
    ...
    const unsigned int numberOfRounds = 9;
    ...
    // Copy the Cipher text to message
    for(i = 0; i < 16; i++) {
        messsage[i] = state[i];
    }
}
...
int main() {
    ...
    unsigned char* paddedMessage = (unsigned char*)malloc(lenOfPaddedMessage * sizeof(unsigned char));
    // unsigned char paddedMessage[lenOfPaddedMessage];
    int i;
    for(i = 0; i < lenOfPaddedMessage; i++) {
        if(i >= originalLen) {
            paddedMessage[i] = 0; // padding with 0
        }
        else {
    ...
    for(i = 0; i < lenOfPaddedMessage; i += 16) {
